  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-black dark:text-white">Drivers</h1>
          <p className="text-gray-500 dark:text-gray-400 text-sm mt-1">{drivers.length} {t.totalDrivers}</p>
        </div>
      ...
      <div className="relative max-w-xs">
        ...
        <input
          className="w-full bg-white dark:bg-gray-800 border border-[#e8d2e2] dark:border-gray-700 rounded-lg pl-9 pr-3 py-2 text-black dark:text-white text-sm placeholder-gray-400 dark:placeholder-gray-500 focus:outline-none focus:border-purple-500"
          placeholder={t.searchDrivers} value={search} onChange={e => setSearch(e.target.value)}
        />
      </div>

      <Table>
        <Thead>
          <Th>{t.name}</Th><Th>{t.licenseNumber}</Th><Th>{t.licenseClass}</Th><Th>{t.licenseExpiry}</Th><Th>{t.phone}</Th><Th>{t.status}</Th><Th>{t.actions}</Th>
        </Thead>
        <Tbody>
          {filtered.length === 0 ? (
            <Tr><Td className="text-center py-8 text-gray-500 dark:text-gray-400" colSpan={7}>{t.noDrivers}</Td></Tr>
          ) : filtered.map(d => (
            <Tr key={d.id}>
              <Td className="font-medium text-black dark:text-white">{d.name}</Td>
              <Td>{d.license_number}</Td>
              <Td>{d.license_class}</Td>
              <Td>
                <span className={isExpired(d.license_expiry) ? 'text-red-400' : isExpiringSoon(d.license_expiry) ? 'text-black dark:text-yellow-400' : 'text-gray-500 dark:text-gray-400'}>
      ...
      <Modal open={modal} onClose={() => setModal(false)} title={editing ? t.editDriver : t.addDriver}>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            <Input label={t.licenseNumber} value={form.license_number} onChange={set('license_number')} required />
            <Input label={t.licenseClass} placeholder="e.g. Class A" value={form.license_class} onChange={set('license_class')} required />
          </div>
          <div className="grid grid-cols-2 gap-3">
            <Input label={t.licenseExpiry} type="date" value={form.license_expiry} onChange={set('license_expiry')} required />
            <Input
              label={t.phone}
              type="tel"
              ...
              maxLength={10}
              pattern="[0-9]{10}"
              error={form.phone && form.phone.length !== 10 ? t.mustBe10Digits : ''}
              required
            />
          </div>
          {editing && (
